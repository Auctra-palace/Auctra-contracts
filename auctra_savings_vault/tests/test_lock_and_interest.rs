...
fn test_withdraw_one_second_before_term_fails() {
    ...
    let result = h.client.try_withdraw(&h.user, &term, &amount);
    assert!(result.is_err(), "Withdrawal 1 second before term must be rejected");
}
...
fn test_interest_accrues_proportionally_to_elapsed_time() {
    ...
    assert_eq!(
        h.user_balance(),
        principal + exp_yield,
        "h.user_balance() should equal principal + exp_yield"
    );
}
...
fn test_zero_yield_rate_pays_no_interest() {
    ...
    h.client.withdraw(&h.user, &term, &principal);
    assert_eq!(h.user_balance(), principal, "h.user_balance() should equal principal");
    // exactly principal, no yield
}
...
fn test_multiple_terms_are_independent() {
    ...
    assert_eq!(
        ...
        long_amount,
        "h.client.get_balance(&h.user, &long_term) should equal long_amount"
    );
}
...
fn test_partial_withdrawal_leaves_correct_remainder() {
    ...
    assert_eq!(
        h.user_balance(),
        withdraw_amount + exp_yield,
        "h.user_balance() should equal withdraw_amount + exp_yield"
    );
    assert_eq!(
        h.client.get_balance(&h.user, &term),
        remaining,
...
fn test_two_users_are_isolated() {
    ...
    assert_eq!(
        soroban_sdk::token::Client::new(&h.env, &h.auctra_token).balance(&user2),
        amount
    , "soroban_sdk::token::Client::new(&h.env, &h.auctra_token).balance(&user2) should equal amount");
...
fn test_withdraw_more_than_deposited_is_rejected() {
    ...
    assert!(result.is_err(), "Withdrawing more than deposited must be rejected");
}
...
fn test_one_year_lock_accumulates_full_annual_yield() {
    ...
    let h = Harness::new(0, yield_rate_bps);
    let term: u64 = SECONDS_PER_YEAR;
    let principal = 100_000_000i128; // 10 Auctra at 7 decimals
...
fn test_second_deposit_after_first_term_earns_independent_yield() {
    ...
    assert_eq!(
        h.client.get_balance(&h.user, &term),
        0,
        "lots must be cleared after full withdrawal"
    );
    ...
    assert_eq!(
        h.user_balance(),
        amount * 2 + exp_yield1 + exp_yield2,
        "h.user_balance() should equal amount * 2 + exp_yield1 + exp_yield2"
    );
}
