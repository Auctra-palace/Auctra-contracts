...
use auctra_savings_vault::DepositEvent;
use auctra_savings_vault::{SavingsVault, SavingsVaultClient, WithdrawEvent};
use shared::DECIMALS;
use soroban_sdk::{
    symbol_short,
...
fn test_withdraw_after_term_has_correct_30day_yield() {
    ...
    client.initialize(&admin, &auctra_token, &fee_rate, &yield_rate);
    ...
    assert_eq!(
        ...
        "token_client.balance(&user) should equal expected_user_payout"
    );
    assert_eq!(
        token_client.balance(&admin),
        expected_fee,
        "token_client.balance(&admin) should equal expected_fee"
    );
    ...
    assert_eq!(
        withdraw_event.yield_amount, expected_yield,
        "withdraw_event.yield_amount should equal expected_yield"
...
fn test_partial_withdraw_and_multiple_deposits_fifo_yield() {
    ...
    assert_eq!(
        ...
        4_000_000,
        "client.get_balance(&user, &term_seconds) should equal 4_000_000"
    );
}
...
fn test_early_withdrawal_is_rejected() {
    ...
    client.initialize(&admin, &auctra_token, &300, &1_000);
    ...
    assert!(result.is_err(), "Withdrawal before term elapsed must be rejected");
    // Balance must still be intact

    assert_eq!(
        client.get_balance(&user, &term_seconds),
        net_deposit,
        "client.get_balance(&user, &term_seconds) should equal net_deposit"
    );
}
...
fn test_withdraw_at_exact_term_boundary_succeeds() {
    ...
    assert_eq!(
        token_client.balance(&user),
        deposit_amount,
        "token_client.balance(&user) should equal deposit_amount"
    );
    assert_eq!(
        client.get_balance(&user, &term_seconds),
        0,
        "client.get_balance(&user, &term_seconds) should equal 0"
    );
}
...
fn test_withdraw_only_uses_lots_that_reached_their_own_term() {
    ...
    let contract_id = env.register_contract(None, SavingsVault);
    let client = SavingsVaultClient::new(&env, &contract_id);
    client.initialize(&admin, &auctra_token, &0i128, &0i128);
    ...
    assert_eq!(
        client.get_balance(&user, &short_term),
        0,
        "client.get_balance(&user, &short_term) should equal 0"
    );
    ...
    assert_eq!(
        client.get_balance(&user, &long_term),
        long_amount,
        "client.get_balance(&user, &long_term) should equal long_amount"
    );
}
...
fn test_deposit_fee_reflected_in_balance() {
    ...
    assert_eq!(
        client.get_balance(&user, &term_seconds),
        expected_net,
        "client.get_balance(&user, &term_seconds) should equal expected_net"
    );
...
fn test_deposit_event_has_fee_fields() {
    ...
    client.initialize(&admin, &auctra_token, &300, &0);
    let token_admin = soroban_sdk::token::StellarAssetClient::new(&env, &auctra_token);
    ...
    assert_eq!(
        deposit_event.gross_amount, DECIMALS,
        "deposit_event.gross_amount should equal DECIMALS"
    );
    assert_eq!(
        deposit_event.fee_amount, 300_000,
        "deposit_event.fee_amount should equal 300_000"
    );
    assert_eq!(
        deposit_event.net_amount, 9_700_000,
        "deposit_event.net_amount should equal 9_700_000"
    );
    assert_eq!(
...
#[test]
fn test_update_auctra_token_by_admin_savings_vault() {
    let env = Env::default();
    env.mock_all_auths();
    let admin = Address::generate(&env);
    let auctra_token = env
    ...
    let contract_id = env.register_contract(None, SavingsVault);
    let client = SavingsVaultClient::new(&env, &contract_id);
    client.initialize(&admin, &auctra_token, &100, &500);
...
fn test_withdraw_event_yield_amount_nonzero_issue_225() {
    ...
    client.initialize(&admin, &auctra_token, &0i128, &yield_rate_bps);
...
fn test_deposit_fails_and_does_not_write_to_storage_when_balance_insufficient() {
    ...
    client.initialize(&admin, &auctra_token, &300, &1000);
