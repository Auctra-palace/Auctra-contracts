...
use auctra_savings_vault::{DepositEvent, SavingsVault, SavingsVaultClient, WithdrawEvent};
...
impl TestEnv {
    fn new(fee_rate_bps: i128, yield_rate_bps: i128) -> Self {
        ...
        let auctra_token = env
            .register_stellar_asset_contract_v2(admin.clone())
            .address();

        let contract_id = env.register_contract(None, SavingsVault);

        let client = SavingsVaultClient::new(unsafe { &*(&env as *const Env) }, &contract_id);

        client.initialize(&admin, &auctra_token, &fee_rate_bps, &yield_rate_bps);
    ...
    fn token_admin(&self) -> soroban_sdk::token::StellarAssetClient {
        soroban_sdk::token::StellarAssetClient::new(&self.env, &self.auctra_token)
    }

    fn token_client(&self) -> soroban_sdk::token::Client {
        soroban_sdk::token::Client::new(&self.env, &self.auctra_token)
    }
    ...
    fn find_withdraw_event(&self) -> Option<WithdrawEvent> {
        ...
        events
            ...
            .find(|e| {
                e.0 == self.contract_id
                    && Symbol::from_val(&self.env, &e.1.get(0).unwrap())
...
fn test_deposit_basic_success() {
    ...
    assert_eq!(
        ...
        amount,
        "h.client.get_balance(&h.user, &term) should equal amount"
    );
...
fn test_deposit_with_fee_deduction() {
    ...
    assert_eq!(
        h.client.get_balance(&h.user, &term),
        expected_net,
        "h.client.get_balance(&h.user, &term) should equal expected_net"
    );
...
fn test_deposit_with_high_fee_rate() {
    ...
    assert_eq!(
        h.client.get_balance(&h.user, &term),
        gross - fee,
        "h.client.get_balance(&h.user, &term) should equal gross - fee"
    );
}
...
fn test_multiple_deposits_different_terms_independent() {
    ...
    assert_eq!(
        h.client.get_balance(&h.user, &term1),
        amount,
        "h.client.get_balance(&h.user, &term1) should equal amount"
    );
    assert_eq!(
        h.client.get_balance(&h.user, &term2),
        amount,
        "h.client.get_balance(&h.user, &term2) should equal amount"
    );
}
...
fn test_deposit_event_correct_values() {
    ...
    let event = h
        .find_deposit_event()
        .expect("Deposit event must be emitted");
    ...
    assert_eq!(
        ...
        h.now() + term,
        "event.maturity_timestamp should equal h.now() + term"
    );
}
...
fn test_withdraw_after_term_succeeds() {
    ...
    assert_eq!(
        h.client.get_balance(&h.user, &term),
        0,
        "h.client.get_balance(&h.user, &term) should equal 0"
    );
}
...
fn test_withdraw_more_than_available_rejected() {
    ...
    assert!(result.is_err(), "Withdrawing more than available must be rejected");
}
...
fn test_partial_withdraw_leaves_remainder() {
    ...
    assert_eq!(
        h.user_balance(),
        withdraw_amount,
        "h.user_balance() should equal withdraw_amount"
    );
    assert_eq!(
        h.client.get_balance(&h.user, &term),
        remaining,
        "h.client.get_balance(&h.user, &term) should equal remaining"
    );
}
...
fn test_partial_withdraws_multiple_times() {
    ...
    assert_eq!(
        h.user_balance(),
        withdraw1 + withdraw2,
        "h.user_balance() should equal withdraw1 + withdraw2"
    );
    ...
    assert_eq!(
        ...
        "h.user_balance() should equal withdraw1 + withdraw2 + withdraw3"
    );
    assert_eq!(
        h.client.get_balance(&h.user, &term),
        0,
...
fn test_withdraw_with_no_deposit_rejected() {
    ...
    assert!(result.is_err(), "Withdraw with no prior deposit must be rejected");
}
...
fn test_multiple_terms_are_independent_and_locked() {
    ...
    assert_eq!(
        h.client.get_balance(&h.user, &short_term),
        0,
        "h.client.get_balance(&h.user, &short_term) should equal 0"
    );
    ...
    assert_eq!(
        h.client.get_balance(&h.user, &long_term),
        amount,
        "h.client.get_balance(&h.user, &long_term) should equal amount"
    );
}
...
fn test_different_terms_have_independent_lock_times() {
    ...
    assert_eq!(
        h.client.get_balance(&h.user, &term2),
        0,
        "h.client.get_balance(&h.user, &term2) should equal 0"
    );
}
...
fn test_multiple_deposits_different_times_same_term() {
    ...
    assert_eq!(
        h.user_balance(),
        amount1 + amount2,
        "h.user_balance() should equal amount1 + amount2"
    );
}
...
fn test_yield_accrues_after_term_only() {
    ...
    assert_eq!(
        ...
        0,
        "h.client.get_pending_yield(&h.user, &term) should equal 0"
    );
    ...
    assert_eq!(
        h.client.get_pending_yield(&h.user, &term),
        exp_yield,
        "h.client.get_pending_yield(&h.user, &term) should equal exp_yield"
    );
}
...
fn test_yield_30_days_at_10_percent_apr() {
    ...
    assert_eq!(
        h.user_balance(),
        amount + exp_yield,
        "h.user_balance() should equal amount + exp_yield"
    );
}
...
fn test_yield_one_year_at_10_percent_apr() {
    ...
    assert_eq!(
        h.user_balance(),
        amount + 1_000_000,
        "h.user_balance() should equal amount + 1_000_000"
    );
}
...
fn test_yield_six_months_at_10_percent_apr() {
    ...
    assert_eq!(
        h.user_balance(),
        amount + 500_000,
        "h.user_balance() should equal amount + 500_000"
    );
}
...
fn test_yield_on_net_deposit_after_fee() {
    ...
    assert_eq!(
        h.user_balance(),
        net + exp_yield,
        "h.user_balance() should equal net + exp_yield"
    );
    assert_eq!(h.admin_balance(), fee, "h.admin_balance() should equal fee");
}
...
fn test_yield_low_rate_5_percent_annual() {
    ...
    let h = TestEnv::new(0, yield_rate);
    let amount = 100_000_000i128; // 10 Auctra
    let term = SECONDS_PER_YEAR;
...
fn test_two_users_independent_deposits_and_yields() {
    ...
    assert_eq!(
        h.user2_balance(),
        amount + exp_yield,
        "h.user2_balance() should equal amount + exp_yield"
    );
}
...
fn test_fifo_withdrawal_from_multiple_deposits() {
    ...
    assert_eq!(
        h.client.get_balance(&h.user, &term),
        lot3,
        "h.client.get_balance(&h.user, &term) should equal lot3"
    );
}
...
fn test_partial_yield_when_withdrawing_before_all_lots_mature() {
    ...
    assert_eq!(
        h.user_balance(),
        amount + exp_yield1,
        "h.user_balance() should equal amount + exp_yield1"
    );
...
fn test_contract_state_consistency_after_partial_withdrawals() {
    ...
    assert_eq!(
        h.client.get_balance(&h.user, &term),
        amount / 2,
        "h.client.get_balance(&h.user, &term) should equal amount / 2"
    );
...
fn test_precision_with_various_combinations() {
    ...
    for (fee_rate, yield_rate) in test_cases {
        ...
        assert_eq!(
            h.user_balance(),
            actual_deposit + exp_yield,
            "h.user_balance() should equal actual_deposit + exp_yield"
        );
...
fn test_yield_continues_accruing_after_term_maturity() {
    ...
    assert_eq!(
        h.client.get_pending_yield(&h.user, &term),
        yield_at_maturity,
        "h.client.get_pending_yield(&h.user, &term) should equal yield_at_maturity"
    );
    ...
    assert_eq!(
        h.client.get_pending_yield(&h.user, &term),
        total_yield,
        "Yield should continue accruing after maturity"
    );
    ...
    assert_eq!(
        h.user_balance(),
        amount + total_yield,
        "h.user_balance() should equal amount + total_yield"
    );
}
...
fn test_multiple_users_different_terms_isolated() {
    ...
    assert_eq!(
        h.user_balance(),
        amount1 + yield1,
        "h.user_balance() should equal amount1 + yield1"
    );
    ...
    assert_eq!(
        h.user2_balance(),
        amount2 + yield2,
        "h.user2_balance() should equal amount2 + yield2"
    );
}
...
fn test_fee_and_yield_with_multiple_partial_withdrawals() {
    ...
    assert_eq!(
        h.client.get_balance(&h.user, &term),
        net,
        "h.client.get_balance(&h.user, &term) should equal net"
    );
    assert_eq!(h.admin_balance(), fee, "h.admin_balance() should equal fee");
    ...
    assert_eq!(
        h.user_balance(),
        net + total_yield,
        "Total withdrawn should equal net + total_yield"
    );
}
...
fn test_concurrent_deposits_and_withdrawals_multiple_terms() {
    ...
    assert_eq!(
        h.user_balance(),
        amount_short + yield_short,
        "h.user_balance() should equal amount_short + yield_short"
    );

    // Medium and long should still be locked
    assert!(h
        .client
        .try_withdraw(&h.user, &medium_term, &amount_medium)
        .is_err());
    assert!(h
        .client
        .try_withdraw(&h.user, &long_term, &amount_long)
        .is_err());
    ...
    assert_eq!(
        h.user_balance(),
        amount_short + yield_short + amount_medium + yield_medium,
        "h.user_balance() should equal amount_short + yield_short + amount_medium + yield_medium"
    );
    ...
    let total =
        amount_short + yield_short + amount_medium + yield_medium + amount_long + yield_long;
