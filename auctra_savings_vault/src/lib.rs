...
/// Mirrors auctra_reserve_tracker's instance-TTL ceiling: bumps the entry to
/// ~300 days (at 5s/ledger) on every deposit/withdraw so admin config
/// (auctra_token, fee_rate, yield_rate) never archives from inactivity.
...
impl SavingsVault {
    ...
    fn load_auctra_token(env: &Env) -> Result<Address, Error> {
        env.storage()
            .instance()
    ...
    /// afterwards.
    ///
    /// Sets the `admin`, the `auctra_token` accepted for deposits, the deposit
    ...
    /// Deposit (lock) Auctra for a term. User transfers Auctra to this contract.
    pub fn deposit(env: Env, user: Address, amount: i128, term_seconds: u64) -> i128 {
        ...
        let auctra = Self::load_auctra_token(&env).unwrap_or_else(|e| env.panic_with_error(e));
        let admin = Self::load_admin(&env).unwrap_or_else(|e| env.panic_with_error(e));
        let token = soroban_sdk::token::Client::new(&env, &auctra);
    ...
    /// Withdraw unlocked Auctra + yield for a specific term.
    pub fn withdraw(env: Env, user: Address, term_seconds: u64, amount: i128) -> i128 {
        ...
        // Yield is paid directly out of the vault's own Auctra balance — there is no
