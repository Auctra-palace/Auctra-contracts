...
use shared::{
    ...
    DECIMALS, MIN_BURN_AMOUNT, ORACLE_GET_AUCTRA_RATE_WITH_TS, ORACLE_GET_BASKET_WEIGHT,
    ORACLE_GET_CURRENCIES, ORACLE_GET_RATE_WITH_TS, ORACLE_GET_S_TOKEN_ADDR, RESERVE_IS_SUFFICIENT,
...
impl BurningContract {
    ...
    pub fn initialize(
        ...
        env.storage()
            .instance()
            .set(&DATA_KEY.auctra_token, &auctra_token);
        env.storage()
            .instance()
    ...
    /// Redeem `auctra_amount` of Auctra for a single basket currency's S-token.
    ///
    /// Burns `auctra_amount` from `user`, deducts the single-redemption fee, then
    /// transfers the equivalent S-token amount to `recipient` from the vault.
    /// Requires that both the Auctra/USD and currency/USD oracle prices are fresh
    ...
    /// before any Auctra is burned. Pass `None` to disable the check.
    pub fn redeem_single(
        ...
        let fee = calculate_fee(auctra_amount, fee_single);
        let net_auctra = auctra_amount
            .checked_sub(fee)
            .expect("Underflow in net auctra calculation");

        let stoken_out = net_auctra
        ...
        Self::check_reserves(&env, &auctra_token, &reserve_tracker_addr);

        let auctra_client = soroban_sdk::token::Client::new(&env, &auctra_token);
        auctra_client.burn(&user, &auctra_amount);
    ...
    /// Redeem Auctra for proportional Afreum S-tokens across the basket (lower fee tier).
    ///
    ...
    /// with `SlippageExceeded` before any Auctra is burned if any leg's computed
    ...
    pub fn redeem_basket(
        ...
        let total_fee = calculate_fee(auctra_amount, fee_rate);
        let net_auctra = auctra_amount
            .checked_sub(total_fee)
            .expect("Underflow in net auctra");
        let usd_total = net_auctra
            .checked_mul(auctra_rate)
        ...
        // transaction reverts cleanly without burning Auctra first.
        if let Some(ref floors) = min_stokens_out {
            ...
            for i in 0..currencies.len() {
                ...
                let (pf_usd_i, pf_gross_i, pf_fee_i) = if pf_last_positive == Some(i) {
                    (
                        ...
                        auctra_amount
                            .checked_sub(pf_allocated_gross)
                            .expect("pf gross"),
                    ...
                    (
                        ...
                        Self::weighted_floor(auctra_amount, weight, total_weight),
                        Self::weighted_floor(total_fee, weight, total_weight),
                    )
                };
                pf_allocated_usd = pf_allocated_usd
                    .checked_add(pf_usd_i)
                    .expect("pf alloc usd");
                pf_allocated_gross = pf_allocated_gross
                    .checked_add(pf_gross_i)
                    .expect("pf alloc gross");
                pf_allocated_fee = pf_allocated_fee
                    .checked_add(pf_fee_i)
                    .expect("pf alloc fee");
        ...
        for i in 0..currencies.len() {
            ...
            let (usd_i, auctra_gross_i, fee_i) = if last_positive_weight_index == Some(i) {
                (
                    usd_total
            ...
            let net_auctra_i = auctra_gross_i
                .checked_sub(fee_i)
                .expect("Underflow in net per-leg");

            let native_i = net_auctra_i
                .checked_mul(auctra_rate)
                .and_then(|v| v.checked_div(rate))
            ...
            let burn_event = BurnEvent {
                ...
                user: user.clone(),
                auctra_amount: auctra_gross_i,
                net_auctra: net_auctra_i,
    ...
    pub fn cancel_admin_transfer(env: Env) {
        ...
        env.events()
            .publish((symbol_short!("adm_cncl"),), (admin, env.ledger().timestamp()));
    }
    ...
    /// Returns the Auctra token contract address.
    pub fn get_auctra_token(env: Env) -> Address {
        env.storage()
            .instance()
            .get(&DATA_KEY.auctra_token)
            .unwrap()
    ...
    pub fn get_oracle(env: Env) -> Address {
        env.storage().instance().get(&DATA_KEY.oracle).unwrap()
    }
    ...
    pub fn get_vault(env: Env) -> Address {
        env.storage().instance().get(&DATA_KEY.vault).unwrap()
    }

    /// Returns the minimum Auctra amount required for a redemption.
    ...
    fn check_reserves(env: &Env, auctra_token: &Address, reserve_tracker_addr: &Address) {
        let current_supply: i128 =
            env.invoke_contract(auctra_token, &Symbol::new(env, TOKEN_GET_TOTAL_SUPPLY), vec![env]);
