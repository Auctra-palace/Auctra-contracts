...
fn test_redeem_basket_success() {
    ...
    ctx.auctra_token.mint(&ctx.user, &burn_amount);
    ...
    let auctra_rate: i128 = DECIMALS; // 1 Auctra = 1 USD
    let ts = env.ledger().timestamp();
    ctx.oracle.set_auctra_rate(&auctra_rate, &ts);
    ...
    let net_auctra = burn_amount - expected_total_fee;

    let expected_out1 = (6000 * net_auctra * auctra_rate) / (BASIS_POINTS * DECIMALS);
    let expected_out2 = (4000 * net_auctra * auctra_rate) / (BASIS_POINTS * DECIMALS);

    assert_eq!(
        amounts.get(0).unwrap(),
        expected_out1,
        "amounts.get(0).unwrap() should equal expected_out1"
    );
    assert_eq!(
        amounts.get(1).unwrap(),
        expected_out2,
        "amounts.get(1).unwrap() should equal expected_out2"
    );
...
fn test_redeem_basket_allocates_weighted_remainder() {
    ...
    let ts = env.ledger().timestamp();
    ctx.oracle.set_auctra_rate(&DECIMALS, &ts);
    ...
    assert_eq!(
        st1.balance(&r1),
        amounts.get(0).unwrap(),
        "st1.balance(&r1) should equal amounts.get(0).unwrap()"
    );
    assert_eq!(
        st2.balance(&r2),
        amounts.get(1).unwrap(),
        "st2.balance(&r2) should equal amounts.get(1).unwrap()"
    );
    assert_eq!(
        st3.balance(&r3),
        amounts.get(2).unwrap(),
        "st3.balance(&r3) should equal amounts.get(2).unwrap()"
    );
}
...
fn test_redeem_basket_exact_min_amount() {
    ...
    ctx.auctra_token.mint(&ctx.user, &min_amount);
    ...
    assert!(amounts.get(0).unwrap() > 0);
    assert_eq!(
        ctx.auctra_token.balance(&ctx.user),
        0,
        "ctx.auctra_token.balance(&ctx.user) should equal 0"
    );
}
...
fn test_redeem_basket_multiple_calls() {
    ...
    let burn_amount: i128 = 100 * DECIMALS;
    ctx.auctra_token.mint(&ctx.user, &(2 * burn_amount));
    ...
    assert_eq!(
        st1.balance(&recipient),
        out1 + out2,
        "st1.balance(&recipient) should equal out1 + out2"
    );

    // User Auctra balance should be 0 after burning all
    assert_eq!(
        ctx.auctra_token.balance(&ctx.user),
        0,
        "ctx.auctra_token.balance(&ctx.user) should equal 0"
...
fn test_redeem_basket_small_amount_no_dust_loss() {
    ...
    assert_eq!(ctx.auctra_token.balance(&ctx.user), 0);
}
