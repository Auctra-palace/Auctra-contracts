...
impl MockOracle {
    ...
    pub fn set_auctra_rate(env: Env, rate: i128, timestamp: u64) {
        env.storage()
            .instance()
            .set(&symbol_short!("auctra_rt"), &rate);
        env.storage()
            .instance()
            .set(&symbol_short!("auctra_ts"), &timestamp);
    }
...
#[contract]
pub struct MockAUCTRAToken;

#[contractimpl]
impl MockAUCTRAToken {
...
pub struct TestContext {
    ...
    pub auctra_token_id: Address,
    pub auctra_token: MockAUCTRATokenClient<'static>,
...
pub fn setup_test(env: &Env) -> TestContext {
    ...
    let auctra_token_id = env.register_contract(None, MockAUCTRAToken);
    let auctra_token = MockAUCTRATokenClient::new(env, &auctra_token_id);
