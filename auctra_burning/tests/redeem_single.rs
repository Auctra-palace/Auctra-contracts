...
fn test_redeem_single_success() {
    ...
    let net_auctra = burn_amount - expected_fee;
    let expected_out = (net_auctra * auctra_rate) / ngn_rate;

    assert_eq!(out, expected_out, "out should equal expected_out");
    assert_eq!(
        stoken_client.balance(&recipient),
        expected_out,
        "stoken_client.balance(&recipient) should equal expected_out"
    );
    assert_eq!(
...
fn test_redeem_single_self_redeem() {
    ...
    assert!(out > 0);
    assert_eq!(
        stoken_client.balance(&ctx.user),
        out,
        "stoken_client.balance(&ctx.user) should equal out"
    );
}
...
fn test_redeem_single_multiple_calls() {
    ...
    assert_eq!(
        stoken_client.balance(&recipient),
        out1 + out2,
        "stoken_client.balance(&recipient) should equal out1 + out2"
    );
...
fn test_redeem_single_stale_auctra_rate() {
    let env = Env::default();
    let ctx = setup_test(&env);
    ...
    // Auctra rate timestamp is stale (0) → first oracle staleness check fails
    ctx.oracle.set_auctra_rate(&DECIMALS, &0);
...
fn test_redeem_single_stale_currency_rate() {
    ...
    // Auctra rate is fresh (matches ledger) so first check passes
    ctx.oracle.set_auctra_rate(&DECIMALS, &100_000);
