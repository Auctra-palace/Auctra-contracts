...
const MIN_POOL_BALANCE: i128 = 1_000_000; // 0.1 Auctra (7 decimals)
...
pub struct LoanData {
    ...
    /// Auctra. Posting Auctra as collateral for an Auctra loan is a no-op — it locks
...
impl LendingPool {
    ...
    pub fn initialize(env: Env, admin: Address, auctra_token: Address, fee_rate_bps: i128) {
        ...
        env.storage()
            .instance()
            .set(&DataKey::AuctraToken, &auctra_token);
        env.storage()
            .instance()
    ...
    /// Deposit `amount` of Auctra into the pool as the caller's lendable liquidity.
    ///
    ...
    pub fn deposit(env: Env, lender: Address, amount: i128) {
        ...
        let auctra_token: Address = env.storage().instance().get(&DataKey::AuctraToken).unwrap();
        ...
        let token = soroban_sdk::token::Client::new(&env, &auctra_token);
    ...
    /// Withdraw `amount` of Auctra from the caller's pool balance.
    ///
    ...
    /// Borrow `amount` of Auctra from a specific `lender`'s liquidity, creating
    ...
    ///
    /// An earlier iteration pulled a `collateral_amount` of Auctra from the
    /// borrower and required `collateral_amount >= amount` before paying out
    /// `amount` of the *same* Auctra token. That is a degenerate arrangement: the
    /// borrower had to already hold — and give up control of — at least as much
    /// Auctra as they received, so the loan extended no purchasing power, and the
