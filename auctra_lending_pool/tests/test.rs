...
fn test_deposit_and_withdraw() {
    ...
    let amount = DECIMALS; // 1000 Auctra

    let token_admin = soroban_sdk::token::StellarAssetClient::new(&env, &auctra_token);
    ...
    assert_eq!(
        ...
        amount,
        "client.get_balance(&lender) should equal amount"
    );
    ...
    assert_eq!(
        ...
        amount,
        "token_client.balance(&lender) should equal amount"
    );
}
...
fn test_borrow_basic() {
    ...
    let contract_id = env.register_contract(None, LendingPool);
    let client = LendingPoolClient::new(&env, &contract_id);
    client.initialize(&admin, &auctra_token, &0);

    let token_admin = StellarAssetClient::new(&env, &auctra_token);
    let token_client = TokenClient::new(&env, &auctra_token);
    ...
    assert_eq!(
        ...
        borrow_amount,
        "token_client.balance(&borrower) should equal borrow_amount"
    );
...
fn test_fee_rate_accrues_into_repayment_due() {
    ...
    client.initialize(&admin, &auctra_token, &fee_rate_bps);
    ...
    assert_eq!(
        loan.accrued_interest, expected_fee,
        "loan.accrued_interest should equal expected_fee"
    );
    assert_eq!(
        loan.total_repayment_due,
        borrow_amount + expected_fee,
        "loan.total_repayment_due should equal borrow_amount + expected_fee"
    );
}
...
fn test_repay_basic() {
    ...
    assert_eq!(
        token_client.balance(&contract_id),
        pool_liquidity,
        "token_client.balance(&contract_id) should equal pool_liquidity"
    );
    ...
    assert_eq!(
        token_client.balance(&borrower),
        0,
        "token_client.balance(&borrower) should equal 0"
    );
}
...
fn test_borrow_exceeds_liquidity_fails() {
    ...
    assert!(result.is_err(), "borrow exceeding pool liquidity must fail");
}
...
fn test_repay_wrong_loan_id_fails() {
    ...
    assert!(result.is_err(), "repay with wrong loan_id must fail");
}
...
fn test_borrow_repay_full_lifecycle() {
    ...
    assert_eq!(
        client.get_balance(&lender),
        pool_liquidity,
        "client.get_balance(&lender) should equal pool_liquidity"
    );
    assert_eq!(
        token_client.balance(&contract_id),
        pool_liquidity,
    ...
    assert_eq!(
        token_client.balance(&lender),
        pool_liquidity,
        "token_client.balance(&lender) should equal pool_liquidity"
    );
    assert_eq!(
        token_client.balance(&contract_id),
        0,
        "token_client.balance(&contract_id) should equal 0"
    );
}
...
fn test_loan_lifecycle_emits_events() {
    ...
    assert_eq!(
        borrow_event_data.creator, borrower,
        "borrow_event_data.creator should equal borrower"
    );
    assert_eq!(
        borrow_event_data.amount, borrow_amount,
        "borrow_event_data.amount should equal borrow_amount"
    );
    assert_eq!(
        borrow_event_data.token, token_id,
        "borrow_event_data.token should equal token_id"
    );
    assert_eq!(
        borrow_event_data.loan_id, loan_id,
        "borrow_event_data.loan_id should equal loan_id"
    );
    ...
    assert_eq!(
        ...
        "loan.amount should equal borrow_amount - repay_amount"
    );
    ...
    assert_eq!(
        repay_event_data.creator, borrower,
        "repay_event_data.creator should equal borrower"
    );
    assert_eq!(
        repay_event_data.amount, repay_amount,
        "repay_event_data.amount should equal repay_amount"
    );
    assert_eq!(repay_event_data.token, token_id, "repay_event_data.token should equal token_id");
    assert_eq!(
        repay_event_data.loan_id, loan_id,
        "repay_event_data.loan_id should equal loan_id"
    );
...
/// Auctra from the borrower, and a borrower holding no Auctra at all must still be
...
fn test_borrow_takes_no_collateral_from_borrower() {
    ...
    // Borrower starts with zero Auctra — under the old same-asset collateral rule
    // (collateral_amount >= amount) this borrow would have been impossible.
    let borrower = Address::generate(&env);
    assert_eq!(token_client.balance(&borrower), 0, "borrower must start with no Auctra");
    ...
    let loan = client
        .get_loan(&borrower, &loan_id)
        .expect("loan must exist");
    assert_eq!(loan.collateral_amount, 0, "collateral_amount is reserved and must remain 0");
}
