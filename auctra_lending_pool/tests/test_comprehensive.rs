...
use auctra_lending_pool::{
    BorrowEvent, LendingPool, LendingPoolClient, LoanCreatedEvent, LoanStatus, RepayEvent,
};
use shared::DECIMALS;
use soroban_sdk::{
    symbol_short,
...
fn setup() -> (Env, LendingPoolClient<'static>, Address, Address, Address) {
    ...
    (env, client, contract_id, admin, auctra_token)
}
...
fn test_deposit_increases_balance() {
    let (env, client, _contract_id, _admin, auctra_token) = setup();
...
fn test_deposit_zero_amount_fails() {
    let (env, client, _contract_id, _admin, _auctra_token) = setup();
...
fn test_multiple_deposits_accumulate() {
    ...
    assert_eq!(
        client.get_balance(&lender),
        amount1 + amount2,
        "client.get_balance(&lender) should equal amount1 + amount2"
    );
}
...
fn test_withdraw_decreases_balance() {
    ...
    assert_eq!(
        ...
        "client.get_balance(&lender) should equal deposit_amount - withdraw_amount"
    );
}
...
fn test_withdraw_leaving_above_minimum_succeeds() {
    ...
    assert_eq!(
        client.get_balance(&lender),
        50 * DECIMALS,
        "client.get_balance(&lender) should equal 50 * DECIMALS"
    );
}
...
fn test_borrow_emits_event() {
    let (env, client, contract_id, _admin, auctra_token) = setup();
        ...
        .find(|e| {
            e.0 == contract_id
                && e.1.first().map_or(false, |t| {
                    if let Ok(symbol_val) = TryIntoVal::<_, Symbol>::try_into_val(&t, &env) {
...
fn test_multiple_borrowers_same_lender() {
    ...
    let loan1 = client
        .get_loan(&borrower1, &1u64)
        .expect("loan1 should exist");
    let loan2 = client
        .get_loan(&borrower2, &2u64)
        .expect("loan2 should exist");
...
fn test_deposit_withdraw_multiple_lenders() {
    ...
    assert_eq!(
        client.get_balance(&lender1),
        amount1,
        "client.get_balance(&lender1) should equal amount1"
    );
    assert_eq!(
        client.get_balance(&lender2),
        amount2,
        "client.get_balance(&lender2) should equal amount2"
    );
    ...
    assert_eq!(
        client.get_balance(&lender1),
        500 * DECIMALS,
        "client.get_balance(&lender1) should equal 500 * DECIMALS"
    );
    assert_eq!(
        client.get_balance(&lender2),
        1_000 * DECIMALS,
        "client.get_balance(&lender2) should equal 1_000 * DECIMALS"
    );
}
...
fn test_repay_interest_accrual() {
    ...
    env.ledger()
        .with_mut(|l| l.timestamp = current_time + 30 * 24 * 60 * 60);
...
fn test_loan_created_event_has_correct_term_seconds() {
    ...
    assert_eq!(event_data.amount, borrow_amount, "event.amount should match borrow_amount");
    assert_eq!(event_data.interest_bps, 300, "event.interest_bps should match fee_rate (300)");
    assert_eq!(
