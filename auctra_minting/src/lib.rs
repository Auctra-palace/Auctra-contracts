...
use shared::{
    calculate_amount_after_fee, calculate_fee, check_oracle_freshness, reentrancy_guard,
    ContractPhase, CurrencyCode, DataKey as SharedDataKey, MintEvent, BASIS_POINTS,
    CONTRACT_VERSION, DECIMALS, MAX_MINT_AMOUNT, MAX_TOTAL_SUPPLY, MIN_MINT_AMOUNT,
    ORACLE_GET_AUCTRA_RATE_WITH_TS, ORACLE_GET_BASKET_WEIGHT, ORACLE_GET_CURRENCIES,
    ORACLE_GET_RATE, ORACLE_GET_RATE_WITH_TS, ORACLE_GET_S_TOKEN_ADDR, RESERVE_IS_SUFFICIENT,
...
const DATA_KEY: DataKey = DataKey {
    ...
    auctra_token: symbol_short!("AUCTRA_TK"),
...
pub enum MintingError {
    ...
    SupplyMismatch = 5025,
    /// The computed Auctra output is below the caller-supplied `min_auctra_out`
...
impl MintingContract {
    ...
    pub fn initialize(env: Env, config: MintingConfig) {
        ...
        env.storage()
            .instance()
            .set(&DATA_KEY.oracle, &config.oracle);
        env.storage()
            .instance()
            .set(&DATA_KEY.reserve_tracker, &config.reserve_tracker);
        env.storage()
            .instance()
            .set(&DATA_KEY.auctra_token, &config.auctra_token);
        env.storage()
            .instance()
        ...
        env.storage()
            .instance()
            .set(&DATA_KEY.treasury, &config.treasury);
        env.storage()
            .instance()
        ...
        env.storage()
            .instance()
            .set(&DATA_KEY.operator, &config.operator);
        env.storage()
            .instance()
    ...
    /// Mint Auctra from USDC deposit (unchanged reserve/oracle flow).
    ///
    /// `min_auctra_out` is an optional slippage guard: if the computed Auctra amount
    ...
    pub fn mint_from_usdc(
        ...
        let auctra_token: Address = env
            .storage()
            .instance()
            .get(&DATA_KEY.auctra_token)
            .unwrap();
        ...
        // Get Auctra rate with timestamp and validate oracle freshness
        let (auctra_rate, oracle_timestamp): (i128, u64) = env.invoke_contract(
            &oracle_addr,
            &Symbol::new(&env, ORACLE_GET_AUCTRA_RATE_WITH_TS),
            vec![&env],
        );
        ...
        let auctra_amount = usdc_after_fee
            .checked_mul(DECIMALS)
            .and_then(|v| v.checked_div(auctra_rate))
        ...
        // issuer or an authorized minter on the Auctra Stellar Asset Contract.
        ...
        auctra_sac.mint(&recipient, &auctra_amount);

        let fee = calculate_fee(usdc_amount, fee_rate);

        let tx_id = generate_unique_tx_id(&env, &recipient, auctra_amount, "mint_usdc");
    ...
    /// Mint Auctra by depositing Afreum-style S-tokens in full basket proportions (lower fee tier).
    ...
    pub fn mint_from_basket(
        ...
        let max_amount: i128 = env
            ...
            .get(&DATA_KEY.max_mint_amount)
            .unwrap();
        if auctra_amount < min_amount || auctra_amount > max_amount {
            env.panic_with_error(MintingError::InvalidMintAmount);
        }
        ...
        let fee_auctra = calculate_fee(auctra_amount, fee_rate);
        let net_mint = auctra_amount
            .checked_sub(fee_auctra)
        ...
        let usd_total: i128 = auctra_amount
            .checked_mul(auctra_rate)
        ...
        // If this contract is not the SAC minter the call will revert.
        let auctra_sac = soroban_sdk::token::StellarAssetClient::new(&env, &auctra_token);
        auctra_sac.mint(&recipient, &net_mint);
        if fee_auctra > 0 {
            auctra_sac.mint(&treasury, &fee_auctra);
        }
    ...
    /// this entrypoint only prices the deposit and credits Auctra from oracle rates.
    pub fn mint_from_single(
        ...
        let auctra_amount = usd_after_fee
            .checked_mul(DECIMALS)
            .and_then(|v| v.checked_div(auctra_rate))
            .expect("Overflow in auctra amount calculation");
        ...
        let tx_id = generate_unique_tx_id(&env, &recipient, auctra_amount, "mint_single");
        let mint_event = MintEvent {
            transaction_id: tx_id,
            user: recipient.clone(),
    ...
    /// contract's balance** (pre-funded demo SAC supply) into `vault`, then mints Auctra to
    ...
    pub fn mint_from_demo_fiat(
        ...
        let fee = calculate_fee(usd_gross, fee_single);
        let tx_id = generate_unique_tx_id(&env, &recipient, auctra_amount, "mint_demo");
        let mint_event = MintEvent {
            transaction_id: tx_id,
            user: recipient.clone(),
            usdc_amount: usd_gross,
    ...
    pub fn mint_from_fiat(
        ...
        // Auctra at an incorrect price.
        let (auctra_rate, auctra_oracle_timestamp): (i128, u64) = env.invoke_contract(
            &oracle_addr,
            &Symbol::new(&env, ORACLE_GET_AUCTRA_RATE_WITH_TS),
            vec![&env],
        );
        if !check_oracle_freshness(&env, auctra_oracle_timestamp, UPDATE_INTERVAL_SECONDS) {
            env.panic_with_error(MintingError::OracleStale);
        }
    ...
    /// the reentrancy guard and paused check, and does NOT mint Auctra — it only
    /// moves pre-funded S-tokens from custody to a recipient.
    pub fn admin_drip_fiat(env: Env, recipient: Address, currency: CurrencyCode, amount: i128) {
        reentrancy_guard::acquire_guard(&env);
        Self::check_paused(&env);
    ...
    /// Overwrite the tracked total Auctra supply with `new_supply` (admin only).
    ///
    ...
    /// or if `new_supply` does not match the Auctra token's actual on-chain
    /// `total_supply()`.
    pub fn sync_supply(env: Env, new_supply: i128) {
        ...
        let auctra_token: Address = env
            ...
            .get(&DATA_KEY.auctra_token)
            .unwrap();
        let token_client = soroban_sdk::token::Client::new(&env, &auctra_token);
    ...
    /// Return the current tracked total Auctra supply (7 decimals).
    ...
    /// Return the maximum Auctra supply cap (defaults to [`MAX_TOTAL_SUPPLY`]).
    ...
    /// Set the maximum Auctra supply cap (admin only). Reverts if paused.
    ...
    pub fn set_fee_rate(env: Env, fee_rate_bps: i128) {
        ...
        env.events().publish((symbol_short!("fee_upd"),), event);
    }
    ...
    pub fn set_fee_single(env: Env, fee_single_bps: i128) {
        ...
        env.events().publish((symbol_short!("fee_sgl"),), event);
    }
    ...
    /// Update the Auctra token contract address (admin only).
    pub fn update_auctra_token(env: Env, new_auctra_token: Address) {
        let admin: Address = env.storage().instance().get(&DATA_KEY.admin).unwrap();
        admin.require_auth();
        let old_auctra_token: Address = env
            .storage()
            .instance()
            .get(&DATA_KEY.auctra_token)
            .unwrap();
        env.storage()
            .instance()
            .set(&DATA_KEY.auctra_token, &new_auctra_token);
        let event = AddressUpdatedEvent {
            old_address: old_auctra_token,
            new_address: new_auctra_token,
            timestamp: env.ledger().timestamp(),
        };
        env.events().publish((symbol_short!("upd_auct"),), event);
    }
