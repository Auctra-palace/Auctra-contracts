...
This document explains how to manage test snapshots in the `auctra_minting` contract to prevent stale snapshot data from causing false test passes.
...
- Event types may be renamed (e.g., `mint` → `mint_auctra`)
...
cargo test test_snapshot_validation --package auctra_minting

# Run all tests to ensure nothing broke
cargo test --package auctra_minting

# Commit new snapshots
git add auctra_minting/test_snapshots/*.json
git add auctra_minting/tests/snapshot_validation.rs
...
if ! cargo test test_snapshot_validation --package auctra_minting --quiet 2>&1 | grep -q "test result: ok"; then
    ...
    echo "Run: cargo test test_snapshot_validation --package auctra_minting -- --nocapture"
    echo "to see details."
    exit 1
fi
```
...
- Or restore from git: `git checkout -- auctra_minting/test_snapshots/`
...
1. **Remove unused snapshots**
   ```bash
   rm -rf auctra_minting/test_snapshots/
   ```
...
- `auctra_minting/tests/snapshot_validation.rs` - Validation logic
- `auctra_minting/test_snapshots/*.json` - Snapshot files
- `auctra_minting/tests/test.rs` - Main test suite
