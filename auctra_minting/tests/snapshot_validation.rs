...
const EXPECTED_EVENT_TYPES: &[&str] = &["mint", "adm_init", "adm_done", "adm_cncl"];
...
pub fn validate_snapshot(path: &Path) -> ValidationResult {
    ...
    if let Some(ledger) = snapshot.get("ledger") {
        if let Some(entries) = ledger.get("ledger_entries").and_then(|e| e.as_array()) {
                                ...
                                if let Some(data) = data_obj.get("data") {
                                    if let Some(contract_data) = data.get("contract_data") {
                                        if let Some(val) = contract_data.get("val") {
                                            if let Some(contract_instance) =
                                                val.get("contract_instance")
                                            {
                                                if let Some(storage) = contract_instance
                                                    .get("storage")
            ...
            let expected_set: HashSet<String> = EXPECTED_STORAGE_KEYS
                .iter()
                .map(|s| s.to_string())
                .collect();
                    ...
                    if let Some(v0) = body.get("v0") {
                        if let Some(topics) = v0.get("topics").and_then(|t| t.as_array()) {
                            if let Some(first_topic) = topics.first() {
                                if let Some(symbol) =
                                    first_topic.get("symbol").and_then(|s| s.as_str())
                                {
                                    if symbol != "fn_call"
                                        && symbol != "fn_return"
                                        && symbol != "error"
...
mod tests {
    ...
    fn test_snapshot_validation() {
        ...
        if !result.is_valid {
            ...
            eprintln!("  2. Delete old snapshots: rm -rf auctra_minting/test_snapshots/*.json");
            ...
            assert!(result.is_valid, "Snapshot validation failed. See stderr for details.");
        }
    }
    ...
    fn test_snapshot_dir_exists() {
        let dir = snapshot_dir();
        assert!(dir.exists(), "Snapshot directory should exist: {}", dir.display());
    }
}
