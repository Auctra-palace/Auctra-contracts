...
This directory contains the test suite for the `auctra_minting` contract, including unit tests, integration tests, and snapshot validation.
...
### Run Specific Test
```bash
cargo test test_initialize --package auctra_minting
```
...
### Verbose Output
```bash
cargo test --package auctra_minting -- --nocapture
```
...
- Contract source (`auctra_minting/src/`)
- Tests (`auctra_minting/tests/`)
- Snapshots (`auctra_minting/test_snapshots/`)
...
### Minting Tests
- `test_mint_from_usdc` - Mint Auctra using USDC
...
- `test_update_auctra_token_by_admin_minting` - Auctra token update
...
**Methods:**
- `get_auctra_usd_rate()` - Returns fixed Auctra/USD rate
...
fn test_your_feature() {
    ...
    init_mint_client(&env, &client, &admin, &oracle, &reserve_tracker,
        &auctra_token, &usdc_token, &admin, &admin, 300, 100);
