...
impl Harness {
    fn new() -> Self {
        ...
        let client = MintingContractClient::new(unsafe { &*(&env as *const Env) }, &contract_id);
...
fn test_id_exactly_min_length_accepted() {
    let h = Harness::new();
    assert!(h.try_mint("ABCD1234"), "ID of exactly 8 characters must be accepted");
}
...
fn test_mint_event_carries_validated_tx_id() {
    ...
    use soroban_sdk::{symbol_short, testutils::Events, FromVal, IntoVal, Symbol};

    let h = Harness::new();
    let tx_id_str = "FLW-TXN-20240101";
    let tx_id = SorobanString::from_str(&h.env, tx_id_str);

    h.client.mint_from_fiat(
        &h.operator,
        &h.recipient,
