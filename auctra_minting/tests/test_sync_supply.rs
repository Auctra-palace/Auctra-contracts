...
/// Returns (admin, operator, auctra_token_address, client).
fn setup(env: &Env) -> (Address, Address, Address, MintingContractClient) {
    ...
    (admin, operator, auctra_token, client)
}
...
fn test_sync_supply_rejects_negative_value() {
    let env = Env::default();
    let (admin, _operator, _auctra_token, client) = setup(&env);
...
fn test_sync_supply_rejects_min_i128() {
    let env = Env::default();
    let (_admin, _operator, _auctra_token, client) = setup(&env);
...
/// The auctra_token SAC is freshly deployed so its total_supply() == 0.
...
fn test_sync_supply_succeeds_when_matching_token_supply() {
    let env = Env::default();
    let (_admin, _operator, auctra_token_addr, client) = setup(&env);

    env.mock_all_auths_allowing_non_root_auth();

    // Mint some Auctra tokens directly via the SAC so total_supply() > 0.
    let recipient = Address::generate(&env);
    let mint_amount: i128 = 1_000 * DECIMALS;
    let auctra_sac = soroban_sdk::token::StellarAssetClient::new(&env, &auctra_token_addr);
    auctra_sac.mint(&recipient, &mint_amount);
...
fn test_sync_supply_rejects_above_max_supply() {
    ...
    client.set_max_supply(&(10 * DECIMALS)); // cap = 10 Auctra

    // Mint 11 Auctra via SAC so total_supply() > cap.
    let recipient = Address::generate(&env);
    let over_cap: i128 = 11 * DECIMALS;
    let auctra_sac = soroban_sdk::token::StellarAssetClient::new(&env, &auctra_token_addr);
    auctra_sac.mint(&recipient, &over_cap);

    // sync_supply(11 Auctra) matches token supply but exceeds cap → MaxSupplyExceeded.
