...
use shared::{
    ...
    ORACLE_GET_AUCTRA_RATE, ORACLE_GET_RATE_WITH_TS, TOKEN_GET_TOTAL_SUPPLY,
};
...
impl ReserveTrackerContract {
    ...
    /// Return the circulating Auctra total supply by cross-contract-calling
    /// `get_total_supply` on the registered Auctra token contract.
    pub fn get_total_supply_from_token(env: &Env) -> i128 {
        let auctra_token_addr: Address = env
            .storage()
            .instance()
    ...
    /// Verify that on-chain reserves are sufficient to back the circulating Auctra supply.
    ///
    /// Total supply is obtained by cross-contract-calling `get_total_supply` on the
    /// registered Auctra token contract.  Previously this used
    /// `auctra_client.balance(&env.current_contract_address())`, which always returned 0
    /// because the reserve tracker holds no Auctra — causing reserve checks to be skipped
    ...
    pub fn verify_reserves(env: Env) -> bool {
        ...
        env.storage()
            .instance()
            .set(&DATA_KEY.last_verify_call, &now);

        let total_auctra_supply = Self::get_total_supply_from_token(&env);
        if total_auctra_supply == 0 {
            env.panic_with_error(ReserveTrackerError::ZeroSupply);
        }
        let result = Self::is_reserve_sufficient(env.clone(), total_auctra_supply);
        env.storage()
            .instance()
    ...
    /// `total_auctra_supply` instead of querying the token contract. Returns `true`
    /// if reserves meet the minimum ratio.
    fn verify_reserves_manual(env: Env, total_auctra_supply: i128) -> bool {
        Self::is_reserve_sufficient(env, total_auctra_supply)
    }
    ...
    /// Check if the total reserves are sufficient to back the Auctra supply
    ...
    /// Return `true` if total reserve USD value backs `total_auctra_supply` at or
    /// above the configured minimum reserve ratio (default 100%).
    ///
    /// Sums the USD value of all stored reserves and compares it against the Auctra
    /// supply valued at the oracle's Auctra/USD rate. Trivially returns `true` when
    /// supply is non-positive or values to zero.
    pub fn is_reserve_sufficient(env: Env, total_auctra_supply: i128) -> bool {
        if total_auctra_supply <= 0 {
            return true;
        }
        ...
        let oracle_addr: Address = env.storage().instance().get(&DATA_KEY.oracle).unwrap();
        let auctra_usd_rate: i128 = env.invoke_contract(
            &oracle_addr,
            &Symbol::new(&env, ORACLE_GET_AUCTRA_RATE),
            Vec::new(&env),
        );

        let total_auctra_usd = total_auctra_supply
            .checked_mul(auctra_usd_rate)
            .expect("Overflow in Auctra USD calculation")
            .checked_div(100_000_000)
            .expect("Division by zero in Auctra USD calculation");
        if total_auctra_usd == 0 {
            return true;
        }
    ...
    pub fn set_custodian(env: Env, custodian: Address) {
        ...
        env.storage()
            .instance()
            .set(&DATA_KEY.custodian, &custodian);
        env.events()
            .publish((symbol_short!("cust_set"),), (custodian,));
    }
    ...
    pub fn submit_attestation(env: Env, root: BytesN<32>) {
        ...
        let now = env.ledger().timestamp();
        env.storage().instance().set(&DATA_KEY.attested_root, &root);
        env.storage().instance().set(&DATA_KEY.attestation_ts, &now);
        env.events()
