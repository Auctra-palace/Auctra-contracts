...
fn test_initialize() {
    ...
    client.initialize(&admin, &validators, &min_signatures, &currencies, &basket_weights);

    let stored_validators = client.get_validators();
    assert_eq!(stored_validators.len(), 3, "stored_validators.len() should equal 3");
    assert_eq!(
        client.get_min_signatures(),
        min_signatures,
        "client.get_min_signatures() should equal min_signatures"
    );
}
    ...
    for event in events.iter() {
            ...
            && Symbol::from_val(&env, &topics.get(0).unwrap()) == symbol_short!("rate_upd")
        {
            ...
            assert_eq!(
                event_data.currency,
                ngn.clone(),
                "event_data.currency should equal ngn.clone()"
            );
            assert_eq!(event_data.rate, 1235000, "event_data.rate should equal 1235000");
            assert_eq!(
                event_data.validator, validator,
                "event_data.validator should equal validator"
            );
            found = true;
            break;
        }
    ...
    for event in events.iter() {
        ...
        if !topics.is_empty() {
            ...
            if event_symbol == symbol_short!("rate_upd") {
                ...
                assert_eq!(
                    event_data.median_rate, 1005000,
                    "event_data.median_rate should equal 1005000"
                );
                assert_eq!(
                    event_data.outlier_rate, 1350000,
                    "event_data.outlier_rate should equal 1350000"
                );
...
fn test_outlier_source_cannot_move_median() {
    ...
    for event in events.iter() {
            ...
            && Symbol::from_val(&env, &topics.get(0).unwrap()) == symbol_short!("outlier")
        {
            let event_data: OutlierDetectionEvent = event.2.into_val(&env);
            assert_eq!(
                event_data.outlier_rate, 5_000_000,
                "event_data.outlier_rate should equal 5_000_000"
            );
            outlier_count += 1;
        }
    }
...
fn test_update_rate_uses_even_source_median_average() {
    ...
    client.update_rate(&validator, &ngn, &1234567i128, &sources, &env.ledger().timestamp());
    let stored_rate = client.get_rate(&ngn);
...
fn test_update_rate_falls_back_to_provided_rate_when_sources_empty() {
    ...
    client.update_rate(&validator, &ngn, &submitted_rate, &sources, &env.ledger().timestamp());
    assert_eq!(
        client.get_rate(&ngn),
        submitted_rate,
        "client.get_rate(&ngn) should equal submitted_rate"
    );
}
...
/// Basket rate (get_auctra_usd_rate_with_ts) must also be blocked when any
...
fn test_stale_basket_component_blocks_auctra_rate() {
    ...
    assert!(result.is_err(), "stale basket component must block auctra rate");
}
...
fn test_rate_not_initialized_before_first_submission() {
    ...
    // get_auctra_usd_rate should fail before first submission
    let result = client.try_get_auctra_usd_rate();
    assert!(result.is_err(), "get_auctra_usd_rate should fail before first submission");

    // get_auctra_usd_rate_with_ts should fail before first submission
    let result = client.try_get_auctra_usd_rate_with_ts();
    assert!(
        result.is_err(),
        "get_auctra_usd_rate_with_ts should fail before first submission"
