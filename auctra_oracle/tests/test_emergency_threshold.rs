...
fn setup_with(
    n_validators: usize,
    min_sigs: u32,
) -> (Env, Address, Vec<Address>, CurrencyCode, OracleContractClient<'static>) {
    let env = make_env();
    env.mock_all_auths();
...
fn test_two_of_three_consensus_grants_bypass() {
    ...
    assert_eq!(
        ...
        0u32,
        "votes should be cleared after bypass"
    );
}
    ...
    assert!(
        client
            .try_update_rate(
                ...
                &ngn,
                &emergency_rate,
                &single_source(&env, emergency_rate),
                &0u64
            )
            .is_err(),
...
fn test_set_min_signatures_clears_pending_votes() {
    ...
    assert_eq!(
        client.get_emergency_vote_count(&ngn),
        0u32,
        "set_min_signatures must clear pending votes"
    );
