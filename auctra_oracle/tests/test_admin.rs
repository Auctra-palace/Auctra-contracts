...
use auctra_oracle::{OracleContract, OracleContractClient, ADMIN_TIMELOCK_SECONDS};
...
fn test_transfer_and_accept_after_timelock() {
    ...
    assert_eq!(
        ...
        "client.get_pending_admin() should equal Some(new_admin.clone())"
    );

    let eta = client.get_pending_admin_eligible_at().unwrap();
    assert_eq!(
        eta,
        env.ledger().timestamp() + ADMIN_TIMELOCK_SECONDS,
        "eta should equal env.ledger().timestamp() + ADMIN_TIMELOCK_SECONDS"
    );
...
fn test_replace_pending_nomination() {
    ...
    assert_eq!(
        client.get_pending_admin(),
        Some(correct_addr.clone()),
        "client.get_pending_admin() should equal Some(correct_addr.clone())"
    );
