...
fn test_update_rate_by_non_validator_fails() {
    ...
    let result =
        client.try_update_rate(&non_validator, &ngn, &rate, &sources, &env.ledger().timestamp());
    assert!(result.is_err());
}
...
fn test_update_rate_with_emergency_deviation_bypasses_interval() {
    ...
    let stored_rate = client.get_rate(&ngn);
    assert_eq!(
        stored_rate, 1_060_000,
        "stored_rate should equal 1_060_000 after 2-of-3 consensus"
    );
}
...
fn test_outlier_detection_filters_bad_source() {
    ...
    assert_eq!(
        event_data.outlier_rate, 1_350_000,
        "event_data.outlier_rate should equal 1_350_000"
    );
}
...
fn test_set_basket_config() {
    ...
    assert_eq!(
        client.get_basket_weight(&usd),
        6000,
        "client.get_basket_weight(&usd) should equal 6000"
    );
    assert_eq!(
        client.get_basket_weight(&eur),
        4000,
        "client.get_basket_weight(&eur) should equal 4000"
    );
}
...
fn test_get_basket_weight_nonexistent_returns_zero() {
    ...
    assert_eq!(
        client.get_basket_weight(&nonexistent),
        0,
        "client.get_basket_weight(&nonexistent) should equal 0"
    );
}
...
fn test_get_auctra_usd_rate_basket_weighted() {
    ...
    assert_eq!(auctra_rate, 1_500_000, "auctra_rate should equal 1_500_000");
}
...
fn test_get_auctra_usd_rate_with_ts() {
    ...
    assert_eq!(
        timestamp,
        env.ledger().timestamp(),
        "timestamp should equal env.ledger().timestamp()"
    );
}
