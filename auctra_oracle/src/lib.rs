...
use shared::{
    ...
    RateUpdateEvent, BASIS_POINTS, CONTRACT_VERSION, EMERGENCY_THRESHOLD_BPS, MAX_VALIDATORS,
...
impl Display for OracleError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let message = match self {
            ...
            Self::InsufficientEmergencyVotes => {
                "emergency vote cast - waiting for N-of-M validator consensus"
            }
...
impl OracleContract {
    ...
    pub fn cast_emergency_vote(env: Env, validator: Address, currency: CurrencyCode, rate: i128) {
        validator.require_auth();
        ...
        let votes: Vec<EmergencyVote> = all_votes.get(currency.clone()).unwrap_or(Vec::new(&env));
    ...
    pub fn get_emergency_vote_count(env: Env, currency: CurrencyCode) -> u32 {
        ...
        let all_votes: Map<CurrencyCode, Vec<EmergencyVote>> = env
            ...
            .get(&DATA_KEY.emergency_votes)
            .unwrap_or(Map::new(&env));
        let votes: Vec<EmergencyVote> = all_votes.get(currency).unwrap_or(Vec::new(&env));
        let mut count: u32 = 0;
        for v in votes.iter() {
    ...
    /// Compute the basket-weighted Auctra/USD rate together with the oldest
    ...
    /// Compute the basket-weighted Auctra/USD rate (7 decimals).
    ///
    /// Same computation as [`Self::get_auctra_usd_rate_with_ts`] without the
    /// timestamp. Panics if no currencies are configured or no fresh rates
    /// contribute.
    pub fn get_auctra_usd_rate(env: Env) -> i128 {
        let basket_weights: Map<CurrencyCode, i128> = env
            .storage()
            .instance()
            .get(&DATA_KEY.basket_weights)
