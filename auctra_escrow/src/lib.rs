...
impl Display for EscrowError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let message = match self {
            ...
            Self::UninitializedAuctraToken => "escrow token not initialized",
...
const MIN_ESCROW_AMOUNT: i128 = 10_000_000; // 10 Auctra (7 decimals)
...
impl Escrow {
    ...
    fn get_auctra_token(env: &Env) -> Result<Address, EscrowError> {
        env.storage()
            .instance()
            .get(&DATA_KEY.auctra_token)
            .ok_or(EscrowError::UninitializedAuctraToken)
    }
    ...
    pub fn initialize(env: Env, admin: Address, auctra_token: Address) {
    ...
    /// Create escrow: payer deposits Auctra, payee can claim after release
    ...
    pub fn create(env: Env, payer: Address, payee: Address, amount: i128, escrow_id: u64) {
        ...
        let auctra = Self::get_auctra_token(&env).unwrap_or_else(|e| env.panic_with_error(e));
        let client = soroban_sdk::token::Client::new(&env, &auctra);
    ...
    /// Release escrow: payee receives Auctra.
    ...
    /// Refund escrow: payer gets Auctra back (admin or dispute resolution, or payer after expiry)
