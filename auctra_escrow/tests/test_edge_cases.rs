...
use auctra_escrow::{Escrow, EscrowClient, EscrowError};
use soroban_sdk::{
...
fn setup(env: &Env) -> (Address, Address, Address, EscrowClient) {
    ...
    env.mock_all_auths();
    client.initialize(&admin, &auctra_token);

    (admin, auctra_token, contract_id, client)
}
...
fn test_create_self_escrow_panics() {
    ...
    let (_admin, auctra_token, _cid, client) = setup(&env);
    let payer = Address::generate(&env);
    let amount = 10_000_000i128;

    mint(&env, &auctra_token, &payer, amount);
...
fn test_duplicate_create_same_payer_same_id_fails() {
    ...
    mint(&env, &auctra_token, &payer, amount * 2);
...
fn test_same_payer_different_ids_are_independent() {
    ...
    assert_eq!(
        token.balance(&payee),
        amount * 2,
        "token.balance(&payee) should equal amount * 2"
    );
}
...
fn test_non_admin_cannot_call_refund() {
    ...
    let (_admin, auctra_token, contract_id, client) = setup(&env);
    let payer = Address::generate(&env);
    let payee = Address::generate(&env);
    ...
    assert!(result.is_err(), "refund must fail when called without admin auth");
}
...
fn test_unpause_restores_create_and_release() {
    ...
    assert!(
        client
            .try_create(&payer, &payee, &amount, &escrow_id)
            .is_err(),
...
fn test_two_payers_same_escrow_id_independent() {
    ...
    mint(&env, &auctra_token, &payer_a, amount_a);
    mint(&env, &auctra_token, &payer_b, amount_b);
