...
fn create_locks_payer_funds() {
    ...
    assert_eq!(
        ...
        amount,
        "token_client.balance(&client.address) should equal amount"
    );
}
...
fn release_pays_payee_and_removes_escrow() {
    ...
    assert_eq!(
        ...
        amount,
        "token_client.balance(&payee) should equal amount"
    );
...
fn refund_returns_funds_to_payer_and_removes_escrow() {
    ...
    assert_eq!(
        ...
        amount,
        "token_client.balance(&payer) should equal amount"
    );
...
fn different_payers_can_reuse_same_escrow_id_without_collision() {
    ...
    assert_eq!(
        token_client.balance(&payee),
        100_000_000,
        "token_client.balance(&payee) should equal 100_000_000"
    );
}
...
fn same_payer_same_escrow_id_is_rejected_until_released() {
    ...
    assert!(client
        .try_create(&payer, &payee, &10_000_000, &escrow_id)
        .is_err());
...
fn test_pause_without_initialize_returns_uninitialized_admin_error() {
    ...
    assert_eq!(
        result,
        Err(Ok(EscrowError::UninitializedAdmin)),
        "pause should fail before admin initialization"
    );
}

#[test]
fn test_update_auctra_token_by_admin_escrow() {
    let env = Env::default();
    env.mock_all_auths();
    let admin = Address::generate(&env);
    let auctra_token = env
